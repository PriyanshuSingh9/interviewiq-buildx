...
export async function parseResume(dataBuffer) {
    // M8: Reject oversized files (10MB max for a resume PDF)
    const MAX_RESUME_SIZE = 10 * 1024 * 1024; // 10MB
    if (!dataBuffer || dataBuffer.byteLength === 0) {
        throw new Error("Empty resume file");
    }
    if (dataBuffer.byteLength > MAX_RESUME_SIZE) {
        throw new Error(`Resume file too large (${(dataBuffer.byteLength / 1024 / 1024).toFixed(1)}MB). Maximum size is 10MB.`);
    }
